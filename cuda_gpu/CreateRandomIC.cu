...
// Create a random initial condition for particles in a 3D space
void CreateRandomIC(const real Max_R, real (*Pos)[3], real (*Vel)[3], real *Mass, const uint Size){
    ...
    */
        for (uint i = 0; i < Size; i++){
            ...
            #ifdef FLOAT8
                for (uint d = 0; d < 3; d++) Radius += pow((Pos[i][d] - centre[d]), 2.0);
                Radius = sqrt(Radius);
            #else
    ...
    #ifdef FLOAT8
        for (uint i = 0; i < Size; i++) Mass[i] = 1.0; 
    #else
        for (uint i = 0; i < Size; i++) Mass[i] = 1.0f;
    #endif
    /*
    #ifdef OPEN_MP
    }
